...
PRINTER_PROFILES = {
    "bedslinger": {"name": "Bed slinger (Ender 3, CR-10)", "speed_multiplier": 1.0, "max_speed": 80, "notes": "Keep speeds moderate. High speeds cause ringing on bed slingers.", "drive_type": "bowden"},
    "corexY": {"name": "CoreXY (Bambu, Voron, Prusa XL)", "speed_multiplier": 1.8, "max_speed": 200, "notes": "CoreXY handles much higher speeds with minimal ringing.", "drive_type": "direct"},
    "delta": {"name": "Delta (Anycubic Kossel)", "speed_multiplier": 1.2, "max_speed": 100, "notes": "Great for tall objects. Keep retraction tuned carefully.", "drive_type": "bowden"},
    "direct": {"name": "Direct drive (Prusa MK4)", "speed_multiplier": 1.3, "max_speed": 120, "notes": "Direct drive handles flexible filaments much better than Bowden.", "drive_type": "direct"}
}
...
def calculate_settings(filament, printer, purpose, nozzle, size, priority, quality_level=5):
    ...
    # filament cross-section area for 1.75mm filament: pi*r^2 (r=0.875mm) ≈ 2.4053 mm^2
    # length(m) = volume(cm^3) / area(mm^2)  [derivation: V_mm3/area_mm2/1000]
    filament_length_m = round(volume_cm3 / (3.14159 * (1.75 / 2) ** 2), 1)

    # Retraction distance depends heavily on extruder type, not just filament.
    # Direct drive needs far less retraction than Bowden — over-retracting on
    # direct drive causes grinding/clicking. Bowden values below are the
    # filament's tuned baseline; direct-drive scales down to ~15% with a floor.
    if p["drive_type"] == "direct":
        retraction = max(round(f["retraction"] * 0.15, 1), 0.4)
    else:
    ...
    return {
        ...
        "retraction": retraction, "retraction_speed": f["retraction_speed"],
